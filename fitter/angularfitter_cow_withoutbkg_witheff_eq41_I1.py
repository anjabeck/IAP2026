...
def projected_eff_weight(df_truth, df_selected, var, bins=50):
    if var == "mKpi":
        xmin, xmax = 0.65, 1.50
    elif var == "q2":
        xmin, xmax = 1.1, 7.0
    ...
    h_truth, _ = np.histogram(df_truth[var], bins=edges)
    h_selected, _ = np.histogram(df_selected[var], bins=edges)

    eps_bar = np.divide(
        h_selected,
        h_truth,
        out=np.zeros_like(h_selected, dtype=float),
        where=h_truth > 0,
    )
    ...
    if len(positive) == 0:
        raise RuntimeError(f"Projected efficiency for {var} is empty.")

    eps_bar[eps_bar <= 0.0] = np.min(positive)

    values = df_selected[var].to_numpy(dtype=float)
    idx = np.searchsorted(edges, values, side="right") - 1
    idx = np.clip(idx, 0, len(eps_bar) - 1)

    return 1.0 / eps_bar[idx]
    
def make_q_norm(df, bins=(8, 8, 16)):
    values = df[["cosh", "cosl", "B_mass"]].to_numpy(dtype=float)

    q_hist_weights = df["fit_weight"].to_numpy(dtype=float)
    q_hist_weights = q_hist_weights
...
def make_unit_norm():
    def unit_norm(x):
        ...
        if arr.ndim == 1:
            ...
            if arr.shape[0] == 3:
                return np.ones(arr.shape[1], dtype=float)
            elif arr.shape[1] == 3:
                return np.ones(arr.shape[0], dtype=float)
            else:
                raise ValueError(f"Unexpected unit_norm input shape: {arr.shape}")

        else:
            raise ValueError(f"Unexpected unit_norm input ndim: {arr.ndim}")

    return unit_norm


def build_custom_cows_3d(pdfs, norm_func, ranges, n_int=500000, seed=0):
    ...
    norm_values = norm_func(points)
    norm_values = np.asarray(norm_values, dtype=float)

    if np.any(norm_values <= 0.0) or np.any(~np.isfinite(norm_values)):
        raise RuntimeError("norm_func has non-positive or non-finite values.")
    ...
    for pdf in pdfs:
        ...
        g_values.append(vals)

    n_pdfs = len(pdfs)
    w_matrix = np.zeros((n_pdfs, n_pdfs), dtype=float)

    for i_pdf in range(n_pdfs):
        for j_pdf in range(n_pdfs):
            integrand = g_values[i_pdf] * g_values[j_pdf] / norm_values
            w_matrix[i_pdf, j_pdf] = volume * np.mean(integrand)

    a_matrix = np.linalg.solve(w_matrix, np.eye(n_pdfs))

    def make_weight_func(component_index):
        def weight_func(x):
            arr = np.asarray(x, dtype=float)

            if arr.ndim == 1:
                if arr.shape[0] != len(ranges):
                    raise ValueError("Expected one 3D point.")
                pts = arr.reshape(len(ranges), 1)

            elif arr.ndim == 2:
                if arr.shape[0] == len(ranges):
                    pts = arr
                elif arr.shape[1] == len(ranges):
                    pts = arr.T
                else:
            ...
            norm_x = norm_func(pts)
            norm_x = np.asarray(norm_x, dtype=float)

            if np.any(norm_x <= 0.0) or np.any(~np.isfinite(norm_x)):
                raise RuntimeError("norm_func has non-positive or non-finite values.")

            out = np.zeros(pts.shape[1], dtype=float)

            for k_pdf, pdf in enumerate(pdfs):
                vals = pdf(pts)
                vals = np.asarray(vals, dtype=float)
                out += a_matrix[component_index, k_pdf] * vals
    ...
    for i_pdf in range(n_pdfs):
        weight_funcs.append(make_weight_func(i_pdf))

    return weight_funcs, w_matrix, a_matrix
...
for i in range(ntoys):
    ...
    # --------------------------------------------
    # Signal-only COWs for Eq. (41): I(m) = 1
    # A0 / App / AS / Aq
    # --------------------------------------------
    ...
    try:
        ...
        weight_funcs, w_matrix, a_matrix = build_custom_cows_3d(
            pdfs=pdf_list,
            norm_func=unit_norm,
            ranges=ranges_3d,
            n_int=500000,
        ...
        print("Custom COW W matrix:")
        print(w_matrix)
        print("Custom COW A matrix:")
        print(a_matrix)
        print("Custom COW W condition number:")
        print(np.linalg.cond(w_matrix))
        print("Custom COW W eigenvalues:")
        print(np.linalg.eigvalsh(w_matrix))

        wA0_cow = weight_funcs[0](data_cow)
        wApp_cow = weight_funcs[1](data_cow)
        wAS_cow = weight_funcs[2](data_cow)
        wAq_cow = weight_funcs[3](data_cow)
        ...
        print("\nraw sums")
        print("expected component yields")
        print("N_A0 =", float(N_A0.value()))
        print("N_App =", float(N_App.value()))
        print("N_AS =", float(N_AS.value()))
        print("N_Aq =", float(N_Aq.value()))

        print("raw / expected")
        print("A0  =", np.sum(wA0_cow) / float(N_A0.value()))
        print("App =", np.sum(wApp_cow) / float(N_App.value()))
        print("AS  =", np.sum(wAS_cow) / float(N_AS.value()))
        print("Aq  =", np.sum(wAq_cow) / float(N_Aq.value()))
        ...
        import traceback
        traceback.print_exc()
        print("Problem with realistic efficiency weighted signal-only COWs Eq. (41), I=1.")
    # --------------------------------------------
    # Reference comparison plots for q2 and mKpi
    # with projected efficiency correction
    # --------------------------------------------
    if i < 3 and wA0_cow is not None and wApp_cow is not None and wAS_cow is not None:
        df_truth_eff = read_truth_signal_sample()
        outdir_cow = f"plots/{args.polynomial}/{name}/reference_cow_eq41_I1_realistic_weighted_signal_only"
        os.makedirs(outdir_cow, exist_ok=True)

        sample_label = "realistic efficiency weighted"
        method_label = r"COW, Eq. (41), $I=1$"
        ...
        effcorr_mKpi = projected_eff_weight(df_truth_eff, datatoy, "mKpi")
        effcorr_q2 = projected_eff_weight(df_truth_eff, datatoy, "q2")

        weight_dict_cow_mKpi = {
            "wA0": wA0_cow * effcorr_mKpi,
            "wApp": wApp_cow * effcorr_mKpi,
            "wS": wAS_cow * effcorr_mKpi,
        }

        weight_dict_cow_q2 = {
            "wA0": wA0_cow * effcorr_q2,
            "wApp": wApp_cow * effcorr_q2,
            "wS": wAS_cow * effcorr_q2,
        }
        ...
        for weight_name, (ref_file, ref_tree) in reference_mapping.items():
            ...
            make_cow_reference_plot(
                datatoy,
                weight_dict_cow_mKpi[weight_name],
                ref_df,
                "mKpi",
                r"$m(K\pi)$ [GeV/$c^2$]",
                f"{outdir_cow}/{i}_{weight_name}_mKpi.pdf",
                reference_label=reference_display[weight_name],
                data_label=f"{component_display[weight_name]} {method_label} projection",
            )

            make_cow_reference_plot(
                datatoy,
                weight_dict_cow_q2[weight_name],
                ref_df,
                "q2",
                r"$q^2$ [GeV$^2/c^4$]",
        ...
        make_cow_extracted_only_plot(
            datatoy,
            wAq_cow * effcorr_mKpi,
            "mKpi",
            r"$m(K\pi)$ [GeV/$c^2$]",
            f"{outdir_cow}/{i}_wAq_mKpi.pdf",
            data_label=r"$A_q$ COW projection, Eq. (41)",
        )
    ...
    # Plot the result
    if i < 3:
        outdir_fit = f"plots/{args.polynomial}/{name}/fit_projections_cow_eq41_I1_realistic_weighted_signal_only"
        ...
        for v, n, l in zip([cosh, cosl], ["cosh", "cosl"], [r"$\cos(\theta_h)$", r"$\cos(\theta_\ell)$"]):
            ...
            plot_projection_with_pull(
                ...
                output_path=f"{outdir_fit}/{i}_{n}_cow_eq41_I1_realistic_weighted_signal_only.pdf",
                data_label="realistic efficiency weighted data",
                line_x=x,
                total_y=Z,
                total_label="Fit",
