...
df_sig["fit_weight"] = 1 / df_sig["efficiency"].to_numpy(dtype=float)
...
for i in range(ntoys):
    ...
    try:
        ...
        print(e)
        print("Problem with signal-only sWeights.")
    try:
        sweights = compute_sweights(pdfsweights, data)
    except Exception as e:
        print(e)
    ...
    if sweights is not None:
        ...
        print("Nsig - component sum =", float(diff))
        assert np.isclose(diff, 0, atol=1e-2)

        eff_w = 1.0 / datatoy["efficiency"].to_numpy(dtype=float)

        raw_sApp = sweights[N_App]
        raw_sA0 = sweights[N_A0]
        raw_sAS = sweights[N_AS]
        raw_sAq = sweights[N_Aq]
        ...
        print("\n[Debug realistic signal-only efficiency-corrected sWeights]")
        print("sum eff-corrected sA0   =", np.sum(sA0))
        print("sum eff-corrected sApp  =", np.sum(sApp))
        print("sum eff-corrected sAS   =", np.sum(sAS))
        print("sum eff-corrected sAq   =", np.sum(sAq))
        print("sum eff-corrected signal sWeights =", np.sum(sA0 + sApp + sAS + sAq))
    else:
        sA0 = None
        sApp = None
        sAS = None
    ...
    # Use raw sWeights on the efficiency-applied sample.
    # This is the "realistic" configuration and is not expected to close to truth.
    # --------------------------------------------
    if i < 3 and sweights is not None:
        
        outdir_sweights = f"plots/{args.polynomial}/{name}/reference_sweights_realistic_signal_only"
        os.makedirs(outdir_sweights, exist_ok=True)

        sample_label = "realistic"
        ...
        for weight_name, (ref_file, ref_tree) in reference_mapping.items():
            ...
            make_cow_reference_plot(
                datatoy,
                weight_dict_sweights[weight_name],
                ref_df,
                "mKpi",
                r"$m(K\pi)$ [GeV/$c^2$]",
                f"{outdir_sweights}/{i}_{weight_name}_mKpi.pdf",
            ...
            make_cow_reference_plot(
                ...
                "q2",
                r"$q^2$ [GeV$^2/c^4$]",
                f"{outdir_sweights}/{i}_{weight_name}_q2.pdf",
                reference_label=reference_display[weight_name],
                data_label=f"{component_display[weight_name]} {method_label} projection ({sample_label})",
            )
    ...
    # Plot the result
    if i < 3:
        outdir_fit = f"plots/{args.polynomial}/{name}/fit_projections_sweights_realistic_signal_only"
        ...
        for v, n, l in zip([cosh, cosl], ["cosh", "cosl"], [r"$\cos(\theta_h)$", r"$\cos(\theta_\ell)$"]):
            ...
            w = datatoy["fit_weight"].to_numpy(dtype=float)
            ...
            plot_projection_with_pull(
                ...
                output_path=f"{outdir_fit}/{i}_{n}_sweights_realistic_signal_only.pdf",
                data_label="realistic data",
                line_x=x,
                total_y=Z,
                total_label="Fit",
        ...
        plot_projection_with_pull(
            ...
            output_path=f"{outdir_fit}/{i}_B_mass_sweights_realistic_signal_only.pdf",
            data_label="realistic data",
            line_x=x_mass,
...
if (
    raw_sA0 is not None
    and raw_sApp is not None
    and raw_sAS is not None
    and raw_sAq is not None
):
    print("\n[Final sWeight check]")
    print("sum raw signal sWeights:", np.sum(raw_sA0 + raw_sApp + raw_sAS + raw_sAq))
    print("N signal:", float(Nsig.value()))
    print("sum corrected signal sWeights:", np.sum(sA0 + sApp + sAS + sAq))
else:
    print("sWeights were not computed successfully, so no final sWeight sums are printed.")
