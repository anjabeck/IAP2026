...
def make_p_norm(df, bins=(10, 10, 15)):
    values = df[["cosh", "cosl", "B_mass"]].to_numpy(dtype=float)

    p_hist_weights = 1/df["fit_weight"].to_numpy(dtype=float)**2
    p_range = [
        (-1.0, 1.0),
        (-1.0, 1.0),
        (5.170, 5.500),
    ...
    hist_p, edges = np.histogramdd(
        values,
        bins=bins,
        range=p_range,
        weights=p_hist_weights,
        density=False,
    )

    positive = hist_p[hist_p > 0.0]
    n_empty = np.sum(hist_p <= 0.0)

    print("p histogram empty bins:", n_empty)
    print("p histogram total bins:", hist_p.size)

    hist_p = hist_p / np.mean(hist_p)
    ...
    def p_norm(x):
        ...
        if arr.ndim == 1:
            ...
            if arr.shape[0] == 3:
                ...
                pts = arr
        #     else:
        #         raise ValueError(f"Unexpected p_norm input shape: {arr.shape}")
        # else:
        #     raise ValueError(f"Unexpected p_norm input ndim: {arr.ndim}")
        ...
        # idx0 = np.clip(idx0, 0, hist_p.shape[0] - 1)
        # idx1 = np.clip(idx1, 0, hist_p.shape[1] - 1)
        # idx2 = np.clip(idx2, 0, hist_p.shape[2] - 1)

        out = hist_p[idx0, idx1, idx2]
...
def make_cow_reference_plot(datatoy, weights, ref_df, var, xlabel, output_path,reference_label="Reference",data_label="Weighted mixed sample",):
...
def build_custom_cows_3d(pdfs, norm_func, ranges, n_int=500000, seed=0):
    ...
    g_values = []
    for pdf in pdfs:
        vals = np.asarray(pdf(points), dtype=float)
    ...
    for i in range(n):
        for j in range(n):
            integrand = g_values[i] * g_values[j] / norm_values
            w_matrix[i, j] = volume * np.mean(integrand)

    a_matrix = np.linalg.solve(w_matrix, np.eye(n))
    ...
    def make_weight(i):
        def weight_func(x):
            ...
            norm_x = np.asarray(norm_func(pts), dtype=float)

            out = np.zeros(pts.shape[1], dtype=float)
            for k, pdf in enumerate(pdfs):
                out += a_matrix[i, k] * np.asarray(pdf(pts), dtype=float)
    ...
    weight_funcs = [make_weight(i) for i in range(n)]
...
for i in range(ntoys):
    ...
    if args.toy:
        ...
        Nsig.set_value(float(datatoy.query("is_signal == 1")["fit_weight"].sum()))
        Nbkg.set_value(float(datatoy.query("is_signal == 0")["fit_weight"].sum()))
    else:
        datatoy = datai.copy()
    ...
    outdir_results = "results_cow_bkg_eff_Iq"
    ...
    try:
        ...
        wBkg_cow = weight_funcs[4](data_cow)

        w_sum = wA0_cow + wApp_cow + wAS_cow + wAq_cow + wBkg_cow
    ...
    if i < 3 and wA0_cow is not None and wApp_cow is not None and wAS_cow is not None and wBkg_cow is not None:
        outdir_cow = f"plots/{args.polynomial}/{name}/reference_cow_multicomp_eff_bkg_Ip_manual_inter"
        ...
        for ref_name, (ref_file, ref_tree) in reference_mapping.items():
            ...
            wBkg_ref_raw = np.asarray(weight_funcs[4](ref_data_cow), dtype=float)
    ...
    # Plot the result
    if i < 3:
        outdir_fit = f"plots/{args.polynomial}/{name}/fit_projections_cow_realistic_withbkg_Ip_manual_inter"
        ...
        plot_projection_with_pull(
            ...
            output_path=f"{outdir_fit}/{i}_{n}B_mass_with_pull_cow_efficiency_eff.pdf",
            data_label="Data",
            line_x=x_mass,
