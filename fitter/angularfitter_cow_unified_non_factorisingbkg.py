...
for i in range(ntoys):
    ...
    case_tag = (
        ...
        f"I{cow_I}_"
        f"bkgSeriesDegree{args.bkg_series_degree}"
    )
    ...
    if i < 3:
        ...
        for v, n, l in zip([cosh, cosl], ["cosh", "cosl"], [r"$\cos(\theta_h)$", r"$\cos(\theta_\ell)$"]):
            ...
            plot_projection_with_pull(
                ...
                output_path=f"{outdir_fit}/{i}_{n}_with_pull_cow.pdf",
                data_label="Data",
                line_x=x,
                total_y=Z,
                total_label="Fit",
        ...
        plot_projection_with_pull(
            ...
            output_path=f"{outdir_fit}/{i}_{n}B_mass_with_pull_cow.pdf",
            data_label="Data",
            line_x=x_mass,
...
if args.toy:
    ...
    for k in pulls.keys():
        ...
        mplhep.histplot(
            zfit.data.Data.from_numpy(obs=x, array=pullsk).to_binned(5000),
            color='black',
            histtype='errorbar',
            xerr=True,
            yerr=True,
        ...
        plt.plot(
            X,
            gauss.pdf(X),
            label=rf'$\mu={mu.value():.2f}({result[mu]["error"]:.2f})$'+'\n'+rf'$\sigma={sig.value():.2f}({result[sig]["error"]:.2f})$',
            color='red',
        )
        plt.legend()
        plt.yticks([])
