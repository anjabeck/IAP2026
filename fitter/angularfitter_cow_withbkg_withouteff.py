...
# def compute_manual_weights_from_fvals(fvals, yields, labels):
#     yields = [float(y) for y in yields]

#     denom = np.zeros_like(fvals[0], dtype=float)

#     for y, f in zip(yields, fvals):
#         denom += y * f
...
#     fvals_valid = [f[valid] for f in fvals]
#     denom_valid = denom[valid]

#     ncomp = len(fvals_valid)

#     info = np.zeros((ncomp, ncomp), dtype=float)

#     for a in range(ncomp):
#         for b in range(ncomp):
#             info[a, b] = np.sum(
#                 event_weights_valid * fvals_valid[a] * fvals_valid[b] / denom_valid**2
#             )
...
#         numerator = np.zeros_like(denom_valid, dtype=float)

#         for b in range(ncomp):
#             numerator += cov[a, b] * fvals_valid[b]

#         w[valid] = numerator / denom_valid
#         weights[label] = w

#     print("\n[Manual multi-component 3D weights] done")

#     for label, y in zip(labels, yields):
#         print(f"  Sum w{label} = {np.sum(weights[label])}")
#         print(f"  N{label}     = {y}")

#     print("  covariance matrix:")
#     print(cov)
#     print("  cond(info) =", np.linalg.cond(info))
...
def plot_bkg_leakage_mkpi(datatoy, wBkg_cow, output_path):
    edges = np.linspace(0.65, 1.50, 51)

    values = datatoy["mKpi"].to_numpy(dtype=float)
    is_sig = datatoy["is_signal"].to_numpy(dtype=bool)

    h_sig, _ = np.histogram(values[is_sig], bins=edges, weights=wBkg_cow[is_sig])
    h_bkg, _ = np.histogram(values[~is_sig], bins=edges, weights=wBkg_cow[~is_sig])
    h_all = h_sig + h_bkg

    norm = np.sum(h_all)

    if np.isclose(norm, 0.0):
        print("Warning: total wBkg histogram sum is zero.")
        return

    plt.figure(figsize=(8, 6))
    plt.step(edges[:-1], h_all / norm, where="post", label="total wBkg")
    plt.step(edges[:-1], h_sig / norm, where="post", label="true signal contribution")
    plt.step(edges[:-1], h_bkg / norm, where="post", label="true bkg contribution")
    plt.xlabel(r"$m(K\pi)$ [GeV/$c^2$]")
    plt.ylabel("Normalized weighted entries")
    plt.legend()
    plt.savefig(output_path)
    plt.close()
...
for i in range(ntoys):
    ...
    outdir_results = "results_cow_bkg_noeff"
    ...
    try:
        ...
        print("data_cow shape:", data_cow.shape)
        print("test A0 shape:", fA0_3d(data_cow).shape)
        print("test App shape:", fApp_3d(data_cow).shape)
        print("test AS shape:", fAS_3d(data_cow).shape)
        print("test Aq shape:", fAq_3d(data_cow).shape)
        print("test Bkg shape:", fBkg_3d(data_cow).shape)
        ...
        wA0_raw = np.asarray(cows5_3d[0](data_cow), dtype=float)
        wApp_raw = np.asarray(cows5_3d[1](data_cow), dtype=float)
        wAS_raw = np.asarray(cows5_3d[2](data_cow), dtype=float)
        wAq_raw = np.asarray(cows5_3d[3](data_cow), dtype=float)
        wBkg_raw = np.asarray(cows5_3d[4](data_cow), dtype=float)
        ...
        wBkg_cow = wBkg_raw[0] if wBkg_raw.ndim == 2 else wBkg_raw

    except Exception:
        import traceback
        traceback.print_exc()
        print("Problem with 3D multi-component COWs test.")
        wA0_cow = None
        wApp_cow = None
        wAS_cow = None
        wAq_cow = None
    ...
    if i < 3 and wA0_cow is not None and wApp_cow is not None and wAS_cow is not None and wBkg_cow is not None:
        outdir_cow = f"plots/{args.polynomial}/{name}/reference_cow_multicomp_noeff"
        ...
        plot_bkg_leakage_mkpi(
            datatoy,
            wBkg_cow,
            f"{outdir_cow}/{i}_wBkg_mKpi_leakage.pdf",
        )
    ...
    if i < 3:
        ...
        outdir_fit = f"plots/{args.polynomial}/{name}/fit_projections_cow_truth_withbkg"
        ...
        for v, n, l in zip([cosh, cosl], ["cosh", "cosl"], [r"$\cos(\theta_h)$", r"$\cos(\theta_\ell)$"]):
            ...
            plot_projection_with_pull(
                ...
                output_path=f"{outdir_fit}/{i}_B_mass_with_pull_cow_Ig_truth_withbkg_noeff.pdf",
                data_label="Data",
                line_x=x,
                total_y=Z,
                total_label="Fit",
        ...
        plot_projection_with_pull(
            ...
            output_path=f"{outdir_fit}/{i}_B_mass_with_pull_cow_withbkg_noeff.pdf",
            data_label="Data",
            line_x=x_mass,
...
print("COW sum all:", np.sum(wA0_cow + wApp_cow + wAS_cow + wAq_cow + wBkg_cow))