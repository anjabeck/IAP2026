...
def make_sweight_reference_plot(
    ...
    if ref_weights is None:
        ...
        var_ref, _ = np.histogram(ref_values, bins=bin_edges)
    else:
        ref_w = np.asarray(ref_weights, dtype=float)[valid_ref]
        h_ref, _ = np.histogram(ref_values, bins=bin_edges, weights=ref_w)
        var_ref, _ = np.histogram(ref_values, bins=bin_edges, weights=ref_w**2)
    ...
    h_ref_sum = h_ref.sum()
    h_ref = h_ref / h_ref_sum
    err_ref = err_ref / abs(h_ref_sum)
...
for i in range(ntoys):
    ...
    # Compute sWeights
    try:
        eff_w = datatoy["fit_weight"].to_numpy(dtype=float)

        sweights = compute_sweights(
            pdfsweights,
            data,
            sample_weight=eff_w,
        )
        ...
        print(e)
        print("Problem with realistic with-background weighted sWeights.")
        continue
    ...
    print("\n[Debug realistic with-background weighted sWeights]")
    print("sum raw sA0   =", np.sum(raw_sA0))
    print("sum raw sApp  =", np.sum(raw_sApp))
    print("sum raw sAS   =", np.sum(raw_sAS))
    print("sum raw sAq   =", np.sum(raw_sAq))
    print("sum raw sBkg  =", np.sum(raw_sBkg))

    print("sum eff-corrected sA0   =", np.sum(sA0), "expected N_A0  =", float(N_A0.value()))
    print("sum eff-corrected sApp  =", np.sum(sApp), "expected N_App =", float(N_App.value()))
    print("sum eff-corrected sAS   =", np.sum(sAS), "expected N_AS  =", float(N_AS.value()))
    print("sum eff-corrected sAq   =", np.sum(sAq), "expected N_Aq  =", float(N_Aq.value()))
    print("sum eff-corrected sBkg  =", np.sum(sBkg), "expected Nbkg  =", float(Nbkg.value()))
    print("sum eff-corrected signal sWeights =", np.sum(sA0 + sApp + sAS + sAq))
    print("Nsig =", float(Nsig.value()))
    print("sum eff-corrected all sWeights =", np.sum(sA0 + sApp + sAS + sAq + sBkg))
    ...
    # --------------------------------------------
    if i < 3:
        ref_outdir = f"plots/{args.polynomial}/{name}/reference_sweights_realistic_withbkg_floatbkgshape"
        ...
        make_sweight_reference_plot(
            ...
            reference_label="Background efficiency-corrected reference",
    ...
    # Plot the result
    if i < 3:
        outdir_fit = f"plots/{args.polynomial}/{name}/fit_projections_sweights_realistic_withbkg_floatbkgshape"
        ...
        for v, n, l in zip([cosh, cosl], ["cosh", "cosl"], [r"$\cos(\theta_h)$", r"$\cos(\theta_\ell)$"]):
            ...
            plot_projection_with_pull(
                ...
                output_path=f"{outdir_fit}/{i}_{n}_sweights_realistic_withbkg.pdf",
        ...
        plot_projection_with_pull(
            ...
            output_path=f"{outdir_fit}/{i}_B_mass_sweights_realistic_withbkg.pdf",
            data_label="truth data",
            line_x=x_mass,
