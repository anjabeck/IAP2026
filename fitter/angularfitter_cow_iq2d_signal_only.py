...
# Read signal-like sample with efficiency applied
df_sig = pd.read_hdf(
...
print("Number of signal-only data points:", len(datai))
...
data = zfit.Data.from_pandas(
    ...
    obs=obs,
    weights=datai["fit_weight"].to_numpy(dtype=float),
)
...
def make_q_norm_3d(sample, efficiency_values, ranges, bins=(20, 20, 60), floor_fraction=1.0e-6):
    ...
    if sample.shape[0] == 3:
        ...
        raise ValueError(f"Expected sample shape (3, N) or (N, 3), got {sample.shape}")

    if np.any(~np.isfinite(efficiency_values)):
        raise ValueError("Efficiency contains NaN or inf before constructing q(m).")

    if np.any(efficiency_values <= 0.0):
        raise ValueError("Efficiency contains non-positive values before constructing q(m).")

    q_weights = 1.0 / efficiency_values**2
    ...
    volume = widths[0][0] * widths[1][0] * widths[2][0]
    norm = np.sum(hist_q) * volume

    if norm <= 0.0:
        raise ValueError("The q(m) histogram normalization is zero.")

    hist_q = hist_q.astype(float) / norm
    positive = hist_q[hist_q > 0.0]

    if len(positive) > 0:
        floor_value = floor_fraction * np.max(positive)
    else:
        floor_value = 1.0e-300

    hist_q = np.maximum(hist_q, floor_value)
    ...
    def q_norm_3d(x):
        ...
        return np.asarray(hist_q[idx0, idx1, idx2], dtype=float).reshape(-1)

    return q_norm_3d, q_weights

def compute_cow_weights_iq_by_uniform_integration(
    data_sample,
    pdf_list,
    q_norm,
    ranges,
    n_integration=300000,
    seed=12345,
):
    data_sample = np.asarray(data_sample, dtype=float)

    if data_sample.shape[0] != 3:
        raise ValueError(f"Expected data_sample shape (3, N), got {data_sample.shape}")

    rng = np.random.default_rng(seed)

    integration_points = np.vstack([
        rng.uniform(low, high, size=n_integration)
    ...
    for pdf in pdf_list:
        g_int.append(np.asarray(pdf(integration_points), dtype=float).reshape(-1))

    g_int = np.asarray(g_int, dtype=float)

    q_int = np.asarray(q_norm(integration_points), dtype=float).reshape(-1)
    q_int = np.maximum(q_int, 1.0e-300)

    wmat = np.zeros((ncomp, ncomp), dtype=float)

    for row in range(ncomp):
        for col in range(ncomp):
            wmat[row, col] = volume * np.mean(
                g_int[row] * g_int[col] / q_int
            )

    amat = np.linalg.solve(wmat, np.eye(ncomp))

    g_data = []

    for pdf in pdf_list:
        g_data.append(np.asarray(pdf(data_sample), dtype=float).reshape(-1))

    g_data = np.asarray(g_data, dtype=float)

    q_data = np.asarray(q_norm(data_sample), dtype=float).reshape(-1)
    q_data = np.maximum(q_data, 1.0e-300)

    weights = []

    for comp in range(ncomp):
        weight = np.zeros(data_sample.shape[1], dtype=float)

        for basis in range(ncomp):
            weight += amat[comp, basis] * g_data[basis] / q_data

        weights.append(weight)

    return weights, wmat, amat


# ----------------------------------------------------------------------
# 2D angular-only I = q test
# ----------------------------------------------------------------------
# This version intentionally removes B_mass from the COW construction.
# For the signal-only test, all angular components share the same mass PDF,
# so including B_mass only adds numerical noise to the q histogram and to the
# W-matrix inversion.
#
# The default q histogram uses weights 1 / efficiency, not 1 / efficiency**2.
# This estimates the efficiency-corrected angular density more gently and
# avoids over-amplifying low-efficiency regions.
Q_EFF_POWER_2D = 2.0
APPLY_EFF_CORRECTION_TO_PROJECTION_2D = True

def zfit_pdf_to_callable_2d_for_cows(zpdf, obs_space):
    def wrapped(x):
        arr = np.asarray(x, dtype=float)

        if arr.ndim == 1:
            if arr.shape[0] != 2:
                raise ValueError("Expected one 2D point.")
            pts = arr.reshape(1, 2)
...
def make_q_norm_2d(sample, efficiency_values, ranges, bins=(30, 30), eff_power=1.0, floor_fraction=1.0e-5):
    sample = np.asarray(sample, dtype=float)
    efficiency_values = np.asarray(efficiency_values, dtype=float)
    ...
    if sample.shape[0] == 2:
        ...
        raise ValueError(f"Expected sample shape (2, N) or (N, 2), got {sample.shape}")

    if np.any(~np.isfinite(efficiency_values)):
        raise ValueError("Efficiency contains NaN or inf before constructing 2D q.")

    if np.any(efficiency_values <= 0.0):
        raise ValueError("Efficiency contains non-positive values before constructing 2D q.")

    q_weights = 1.0 / efficiency_values**eff_power
    ...
    volume = widths[0][0] * widths[1][0]
    norm = np.sum(hist_q) * volume

    if norm <= 0.0:
        raise ValueError("The 2D q histogram normalization is zero.")
    ...
    def q_norm_2d(x):
        ...
        return np.asarray(hist_q[idx0, idx1], dtype=float).reshape(-1)

    return q_norm_2d, q_weights


def compute_cow_weights_iq_2d_by_uniform_integration(
    data_sample,
    pdf_list,
    q_norm,
    ranges,
    ...
    if data_sample.shape[0] != 2:
        raise ValueError(f"Expected data_sample shape (2, N), got {data_sample.shape}")
...
def compute_cow_weights_with_custom_norm(sample, pdf_list, norm_func, sample_weights=None):
    sample = np.asarray(sample, dtype=float)
    ...
    ncomp = len(pdf_list)
    ndata = sample.shape[1]

    if sample_weights is None:
        sample_weights = np.ones(ndata, dtype=float)
    else:
        sample_weights = np.asarray(sample_weights, dtype=float)

    if sample_weights.shape[0] != ndata:
        raise ValueError("sample_weights has a different length from sample.")

    sample_weights = sample_weights / np.sum(sample_weights)

    pdf_values = []
    for pdf in pdf_list:
        pdf_values.append(np.asarray(pdf(sample), dtype=float).reshape(-1))

    pdf_values = np.asarray(pdf_values, dtype=float)
    norm_values = np.asarray(norm_func(sample), dtype=float).reshape(-1)
    norm_values = np.maximum(norm_values, 1.0e-300)
    ...
    for comp in range(ncomp):
        weight = np.zeros(ndata, dtype=float)

        for basis in range(ncomp):
            weight += amat[comp, basis] * pdf_values[basis] / norm_values
...
for i in range(ntoys):
    ...
    outdir_results = "results_cow_nobkg_eff_Iq2D"
    ...
    print(corr_outname)
    # --------------------------------------------
    # 2D angular-only COWs test with I = q(cosh, cosl)
    # A0 / App / AS / Aq
    # --------------------------------------------
    try:
        data_cow = datatoy[["cosh", "cosl"]].to_numpy(dtype=float).T

        fA0_2d = zfit_pdf_to_callable_2d_for_cows(pdfA0, angles)
        fApp_2d = zfit_pdf_to_callable_2d_for_cows(pdfApp, angles)
        fAS_2d = zfit_pdf_to_callable_2d_for_cows(pdfS, angles)
        fAq_2d = zfit_pdf_to_callable_2d_for_cows(pdfAq, angles)
        ...
        eff_values = datatoy["efficiency"].to_numpy(dtype=float)

        q_norm_2d, q_weights = make_q_norm_2d(
            sample=data_cow,
            efficiency_values=eff_values,
            ranges=cow_ranges,
            bins=(30, 30),
            eff_power=Q_EFF_POWER_2D,
        )
        ...
        weights_q, wmat_q, amat_q = compute_cow_weights_iq_2d_by_uniform_integration(
            data_sample=data_cow,
            pdf_list=pdf_list_q,
            q_norm=q_norm_2d,
            ranges=cow_ranges,
            n_integration=300000,
        ...
        wA0_cow = weights_q[0]
        wApp_cow = weights_q[1]
        wAS_cow = weights_q[2]
        wAq_cow = weights_q[3]

        print("\n[Debug COWs with 2D I=q(cosh, cosl), uniform integration]")
        print("2D q uses q_weights = 1 / efficiency**", Q_EFF_POWER_2D)
        print("q_norm min/max on data:", np.min(q_norm_2d(data_cow)), np.max(q_norm_2d(data_cow)))
        print("W matrix with 2D I=q:")
        print(wmat_q)
        print("A matrix with 2D I=q:")
        print(amat_q)

        w_sum = wA0_cow + wApp_cow + wAS_cow + wAq_cow

        print("COW 2D I=q event-wise sum mean:", np.mean(w_sum))
        print("COW 2D I=q event-wise sum min:", np.min(w_sum))
        print("COW 2D I=q event-wise sum max:", np.max(w_sum))
        print("COW 2D I=q total sum:", np.sum(w_sum))
        print("N signal:", float(Nsig.value()))

    except Exception:
        import traceback
        traceback.print_exc()
        print("Problem with 2D angular-only COWs I=q test.")
        wA0_cow = None
        wApp_cow = None
        wAS_cow = None
        wAq_cow = None
    ...
    if i < 3 and wA0_cow is not None and wApp_cow is not None and wAS_cow is not None:
        outdir_cow = f"plots/{args.polynomial}/{name}/reference_cow_Iq2D_realistic_weighted_signal_only"
        os.makedirs(outdir_cow, exist_ok=True)

        sample_label = "efficiency-weighted signal-only"
        cow_label = r"COW, $I=q_{2D}$"
        ...
        for ref_name, (ref_file, ref_tree) in reference_mapping.items():
            ...
            # wBkg_ref_raw = np.asarray(cows5_3d[4](ref_data_cow), dtype=float)
            # wBkg_ref = wBkg_ref_raw[0] if wBkg_ref_raw.ndim == 2 else wBkg_ref_raw
        ...
        if APPLY_EFF_CORRECTION_TO_PROJECTION_2D:
            weight_dict_cow = {
                "wA0": wA0_cow * effcorr,
                "wApp": wApp_cow * effcorr,
                "wS": wAS_cow * effcorr,
            }
            wAq_plot = wAq_cow * effcorr
        else:
            weight_dict_cow = {
                "wA0": wA0_cow,
                "wApp": wApp_cow,
        ...
        for weight_name, (ref_file, ref_tree) in reference_mapping.items():
            ...
            make_cow_reference_plot(
                ...
                data_label=f"{component_display[weight_name]} {cow_label} projection ({sample_label})",
            )
    ...
    # Plot the result
    if i < 3:
        outdir_fit = f"plots/{args.polynomial}/{name}/fit_projections_cow_Iq2D_realistic_weighted_signal_only"
        ...
        for v, n, l in zip([cosh, cosl], ["cosh", "cosl"], [r"$\cos(\theta_h)$", r"$\cos(\theta_\ell)$"]):
            ...
            plot_projection_with_pull(
                ...
                output_path=f"{outdir_fit}/{i}_{n}_cow_Iq2D_realistic_weighted_signal_only.pdf",
                data_label="efficiency-weighted data",
                line_x=x,
                total_y=Z,
                total_label="Fit",
        ...
        plot_projection_with_pull(
            ...
            output_path=f"{outdir_fit}/{i}_B_mass_cow_Iq2D_realistic_weighted_signal_only.pdf",
            data_label="efficiency-weighted data",
            line_x=x_mass,
...
print("COW 2D I=q sum all:", np.sum(wA0_cow + wApp_cow + wAS_cow + wAq_cow))
print("N signal:", float(Nsig.value()))
