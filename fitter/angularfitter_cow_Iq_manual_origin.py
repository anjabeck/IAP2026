...
def make_q_norm(datatoy):
    values = datatoy[["cosh", "cosl", "B_mass"]].to_numpy(dtype=float)

    weights = datatoy["fit_weight"].to_numpy(dtype=float)**2
    ...
    if len(positive) == 0:
        raise RuntimeError("q histogram is empty.")

    hist_q[hist_q <= 0.0] = np.min(positive)
    ...
    def q_norm(x):
        ...
        i0 = np.searchsorted(edges[0], x[:, 0], side="right") - 1
        i1 = np.searchsorted(edges[1], x[:, 1], side="right") - 1
        i2 = np.searchsorted(edges[2], x[:, 2], side="right") - 1

        i0 = np.clip(i0, 0, hist_q.shape[0] - 1)
        i1 = np.clip(i1, 0, hist_q.shape[1] - 1)
        i2 = np.clip(i2, 0, hist_q.shape[2] - 1)

        return hist_q[i0, i1, i2]

    return q_norm


def make_cow_weights_with_q(datatoy, pdf_list, q_norm, n_mc=500000, seed=0):
    rng = np.random.default_rng(seed)

    mc = np.column_stack([
        rng.uniform(-1.0, 1.0, n_mc),
        rng.uniform(-1.0, 1.0, n_mc),
        rng.uniform(5.170, 5.500, n_mc),
    ])

    volume = 2.0 * 2.0 * (5.500 - 5.170)

    q_mc = q_norm(mc)
    q_mc = np.asarray(q_mc, dtype=float).reshape(-1)

    positive_q = q_mc[q_mc > 0.0]

    if len(positive_q) == 0:
        raise RuntimeError("q_norm is zero everywhere in the MC integration sample.")

    q_mc[q_mc <= 0.0] = np.min(positive_q)

    W = np.zeros((len(pdf_list), len(pdf_list)), dtype=float)

    pdf_mc_values = []

    for pdf in pdf_list:
        vals = np.asarray(pdf(mc), dtype=float).reshape(-1)
        pdf_mc_values.append(vals)

    for i in range(len(pdf_list)):
        for j in range(len(pdf_list)):
            W[i, j] = volume * np.mean(pdf_mc_values[i] * pdf_mc_values[j] / q_mc)

    A = np.linalg.inv(W)

    x = datatoy[["cosh", "cosl", "B_mass"]].to_numpy(dtype=float)

    q_x = q_norm(x)
    q_x = np.asarray(q_x, dtype=float).reshape(-1)

    positive_qx = q_x[q_x > 0.0]

    if len(positive_qx) == 0:
        raise RuntimeError("q_norm is zero everywhere in the data sample.")

    q_x[q_x <= 0.0] = np.min(positive_qx)

    g_x = np.column_stack([
        np.asarray(pdf(x), dtype=float).reshape(-1)
        for pdf in pdf_list
    ])

    weights = g_x @ A.T
    weights = weights / q_x[:, None]
...
for i in range(ntoys):
    ...
    try:
        ...
        weights_q, W_q, A_q = make_cow_weights_with_q(
            datatoy,
            pdf_list_3d,
            q_norm,
            n_mc=500000,
        ...
        print("W matrix for I=q:")
        print(W_q)

        print("A matrix for I=q:")
        print(A_q)
        ...
        print("eff-corrected sums")
        print(
            np.sum(wA0_cow * effcorr),
            np.sum(wApp_cow * effcorr),
            np.sum(wAS_cow * effcorr),
            np.sum(wAq_cow * effcorr),
        )
        ...
        print(
            ...
            np.sum(w_sum),
            "eff-corrected total =",
            np.sum(w_sum * effcorr),
        )

    except Exception:
        import traceback
        traceback.print_exc()
        print("Problem with realistic efficiency weighted signal-only COWs I=q.")
    # --------------------------------------------
    # Reference comparison plots for q2 and mKpi
    # with projected efficiency correction, Eq. (41)
    # --------------------------------------------
    if i < 3 and wA0_cow is not None and wApp_cow is not None and wAS_cow is not None:
        ...
        outdir_cow = f"plots/{args.polynomial}/{name}/reference_cow_Iq_realistic_weighted_signal_only"
        os.makedirs(outdir_cow, exist_ok=True)

        sample_label = "realistic efficiency weighted"
        method_label = r"COW, $I=q$"
        ...
        for weight_name, (ref_file, ref_tree) in reference_mapping.items():
            ...
            make_cow_reference_plot(
                datatoy,
                weight_dict_cow[weight_name] * effcorr_mKpi,
                ref_df,
                "mKpi",
                r"$m(K\pi)$ [GeV/$c^2$]",
                f"{outdir_cow}/{i}_{weight_name}_mKpi_eq41.pdf",
                reference_label=reference_display[weight_name],
                data_label=f"{component_display[weight_name]} {method_label} projection, Eq. (41)",
            )

            make_cow_reference_plot(
                datatoy,
                weight_dict_cow[weight_name] * effcorr_q2,
                ref_df,
                "q2",
                r"$q^2$ [GeV$^2/c^4$]",
                f"{outdir_cow}/{i}_{weight_name}_q2_eq41.pdf",
                reference_label=reference_display[weight_name],
                data_label=f"{component_display[weight_name]} {method_label} projection, Eq. (41)",
            )
        make_cow_extracted_only_plot(
            ...
            "mKpi",
            r"$m(K\pi)$ [GeV/$c^2$]",
            f"{outdir_cow}/{i}_wAq_mKpi_eq41.pdf",
        ...
        make_cow_extracted_only_plot(
            ...
            "q2",
            r"$q^2$ [GeV$^2/c^4$]",
            f"{outdir_cow}/{i}_wAq_q2_eq41.pdf",
            data_label=r"$A_q$ COW projection, Eq. (41)",
        )
    # Plot the result
    if i < 3:
        outdir_fit = f"plots/{args.polynomial}/{name}/fit_projections_cow_Iq_realistic_weighted_signal_only"
        ...
        for v, n, l in zip([cosh, cosl], ["cosh", "cosl"], [r"$\cos(\theta_h)$", r"$\cos(\theta_\ell)$"]):
            ...
            plot_projection_with_pull(
                ...
                output_path=f"{outdir_fit}/{i}_{n}_cow_Iq_realistic_weighted_signal_only.pdf",
                data_label="realistic efficiency weighted data",
                line_x=x,
                total_y=Z,
                total_label="Fit",
