...
for i in range(ntoys):
    ...
    outdir_results = "results_cow_truth_signal_only"
    ...
    if i < 3 and wA0_cow is not None and wApp_cow is not None and wAS_cow is not None:
        outdir_cow = f"plots/{args.polynomial}/{name}/reference_cow_Ig_truth_signal_only"
    ...
    # Plot the result
    if i < 3:
        outdir_fit = f"plots/{args.polynomial}/{name}/fit_projections_cow_Ig_truth_signal_only"
        ...
        for v, n, l in zip([cosh, cosl], ["cosh", "cosl"], [r"$\cos(\theta_h)$", r"$\cos(\theta_\ell)$"]):
            ...
            plot_projection_with_pull(
                ...
                output_path=f"{outdir_fit}/{i}_{n}_cow_Ig_truth_signal_only.pdf",
        ...
        plot_projection_with_pull(
            ...
            output_path=f"{outdir_fit}/{i}_B_mass_cow_Ig_truth_signal_only.pdf",
            data_label="truth data",
            line_x=x_mass,
...
print("COW sum all:", np.sum(wA0_cow + wApp_cow + wAS_cow + wAq_cow))
