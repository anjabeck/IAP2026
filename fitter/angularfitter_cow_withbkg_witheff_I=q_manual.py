...
def plot_projection_with_pull(
    ...
    ax1.legend(loc="best", handlelength=1.5, fontsize=15)
...
def build_q_norm_3d(
    df,
    columns=("cosh", "cosl", "B_mass"),
    weight_column="fit_weight",
    ranges=((-1.0, 1.0), (-1.0, 1.0), (5.170, 5.500)),
    bins=(3, 3, 6),
):
    points = df[list(columns)].to_numpy(dtype=float)
    eff_weight = df[weight_column].to_numpy(dtype=float)

    q_weights = eff_weight**2
    ...
    positive = hist_q[np.isfinite(hist_q) & (hist_q > 0)]

    if len(positive) == 0:
        raise ValueError("q histogram has no positive bins.")

    print("hist_q raw min =", np.nanmin(hist_q))
    print("hist_q raw max =", np.nanmax(hist_q))
    print("hist_q raw mean =", np.nanmean(hist_q))
    print("hist_q raw median =", np.nanmedian(hist_q))
    print("positive bins =", len(positive))
    print("total bins =", hist_q.size)
    print("zero bins =", np.sum(hist_q == 0))
    print("nan bins =", np.sum(~np.isfinite(hist_q)))

    for p in [0, 0.1, 1, 5, 10, 50]:
        print(f"hist_q raw percentile {p}% =", np.percentile(positive, p))
    ...
    def q_norm(x):
        ...
        for axis, edge in enumerate(edges):
            this_idx = np.searchsorted(edge, pts[:, axis], side="right") - 1
            this_idx = np.clip(this_idx, 0, len(edge) - 2)
            bin_indices.append(this_idx)
    ...
    q_data = q_norm(df[list(columns)].to_numpy(dtype=float))

    print("\n[Debug q_norm on input df]")
    print("q(data) min =", np.nanmin(q_data))
    print("q(data) max =", np.nanmax(q_data))
    print("q(data) mean =", np.nanmean(q_data))
    print("q(data) median =", np.nanmedian(q_data))
    print("q(data) zero =", np.sum(q_data == 0))
    print("q(data) nan =", np.sum(~np.isfinite(q_data)))

    q_data_pos = q_data[np.isfinite(q_data) & (q_data > 0)]

    if len(q_data_pos) > 0:
        for p in [0, 0.1, 1, 5, 10, 50]:
            print(f"q(data) percentile {p}% =", np.percentile(q_data_pos, p))

    for cut in [1.0e-6, 1.0e-5, 1.0e-4, 1.0e-3, 1.0e-2]:
        print(f"q(data) < {cut} =", np.sum(q_data < cut))

    return q_norm, hist_q, edges

def make_weighted_hist_1d(values, bin_edges, weights=None):
    H = hist.Hist(
        hist.axis.Variable(bin_edges),
...
def make_cow_reference_plot(
    ...
    H_ref, h_ref, err_ref = make_weighted_hist_1d(
        ref_values,
        bin_edges,
        weights=ref_w,
    )

    H_w, h_w, err_w = make_weighted_hist_1d(
        values,
        bin_edges,
        weights=w,
    )
...
for i in range(ntoys):
    ...
    outdir_results = "results_cow_bkg_eff"
    ...
    try:
        ...
        for name_comp, pdf in zip(
            ...
            print("zero =", np.sum(vals == 0))
            print("nan =", np.sum(~np.isfinite(vals)))

        q_norm, q_hist, q_edges = build_q_norm_3d(
            datatoy,
            columns=("cosh", "cosl", "B_mass"),
            weight_column="fit_weight",
            ranges=ranges_cow,
        ...
        q_test = q_norm(data_cow)

        print("\n[Debug q_norm on data]")
        print("min =", np.min(q_test))
        print("max =", np.max(q_test))
        print("mean =", np.mean(q_test))
        print("zero =", np.sum(q_test <= 0))
        print("nan =", np.sum(~np.isfinite(q_test)))
        ...
        print("\n[Debug component response on reference samples]")

        component_weight_funcs = {
            "wA0": cow[0],
            "wApp": cow[1],
            "wS": cow[2],
            "wAq": cow[3],
        ...
        for ref_name, (ref_file, ref_tree) in reference_mapping.items():
            ...
            if ref_name == "wA0":
                wA0_ref = cow[0](ref_data_cow)

                bin_edges = np.linspace(0.65, 1.50, 51)
                bin_centers = 0.5 * (bin_edges[:-1] + bin_edges[1:])

                sum_w, _ = np.histogram(
                    ref_check["mKpi"].to_numpy(dtype=float),
                    bins=bin_edges,
                    weights=wA0_ref,
                )
                ...
                response = np.zeros_like(bin_centers, dtype=float)
                mask = counts > 0
                response[mask] = sum_w[mask] / counts[mask]

                print("\n[Debug A0 response vs mKpi on pure A0 reference]")
                print("response min =", np.min(response[mask]))
                print("response max =", np.max(response[mask]))
                print("response mean =", np.mean(response[mask]))
                print("response median =", np.median(response[mask]))

                for b, r, c in zip(bin_centers, response, counts):
                    if c > 0:
                        print("mKpi =", b, "response =", r, "N =", c)

            print("\nReference sample:", ref_name)
            print("N =", len(ref_check))

            for wname, wfunc in component_weight_funcs.items():
                wref = wfunc(ref_data_cow)
        ...
        print("\n[Debug q norm]")
        print("q hist min =", np.min(q_hist))
        print("q hist max =", np.max(q_hist))
        print("q hist zero bins =", np.sum(q_hist <= 0))
        print("q hist total bins =", q_hist.size)

        q_test = q_norm(data_cow)
        print("q(data) min =", np.min(q_test))
        print("q(data) max =", np.max(q_test))
        print("q(data) nan =", np.sum(~np.isfinite(q_test)))
    ...
    if i < 3 and wA0_cow is not None and wApp_cow is not None and wAS_cow is not None and wBkg_cow is not None:
        outdir_cow = f"plots/{args.polynomial}/{name}/reference_cow_multicomp_eff_bkg_manual_I=q"
    ...
    # Plot the result
    if i < 3:
        outdir_fit = f"plots/{args.polynomial}/{name}/fit_projections_cow_realistic_withbkg_manual_I=q"
