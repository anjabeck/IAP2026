...
def make_projected_efficiency_weight(
    truth_df,
    realistic_df,
    var,
    values,
    ...
    h_truth, _ = np.histogram(
        truth_df[var].to_numpy(dtype=float),
        bins=bin_edges,
    )

    h_realistic, _ = np.histogram(
        realistic_df[var].to_numpy(dtype=float),
        bins=bin_edges,
    )

    eff = np.zeros(nbins, dtype=float)
    mask = h_truth > 0
    eff[mask] = h_realistic[mask] / h_truth[mask]

    eff = np.clip(eff, min_eff, None)

    indices = np.digitize(values, bin_edges) - 1
    indices = np.clip(indices, 0, nbins - 1)

    projected_weight = 1.0 / eff[indices]

    return projected_weight

def make_weighted_reference_plot(datatoy, weights, ref_df, var, xlabel, output_path, reference_label="Reference", data_label="Weighted sample"):
...
for i in range(ntoys):
    ...
    print("sum all sWeights =", np.sum(sA0 + sApp + sAS + sAq))
    ...
    # --------------------------------------------
    if i < 3:
        outdir_sweights = f"plots/{args.polynomial}/{name}/reference_sweights_truth_signal_only"
    ...
    # Plot the result
    if i < 3:
        outdir_fit = f"plots/{args.polynomial}/{name}/fit_projections_sweights_truth_signal_only"
        ...
        for v, n, l in zip([cosh, cosl], ["cosh", "cosl"], [r"$\cos(\theta_h)$", r"$\cos(\theta_\ell)$"]):
            ...
            plot_projection_with_pull(
                ...
                output_path=f"{outdir_fit}/{i}_{n}_sweights_truth_signal_only.pdf",
                data_label="truth data",
                line_x=x,
                total_y=Z,
                total_label="Fit",
        ...
        plot_projection_with_pull(
            ...
            ylabel=fr"Data points / {bin_width_mass:.4f}",
                        output_path=f"{outdir_fit}/{i}_B_mass_sweights_truth_signal_only.pdf",
            data_label="truth data",
            line_x=x_mass,
