...
for i in range(ntoys):
    ...
    try:
        ...
        w_sum_raw = wA0_cow + wApp_cow + wAS_cow + wAq_cow
        w_sum_final = wA0_final + wApp_final + wAS_final + wAq_final

        print("\n[Debug realistic efficiency weighted signal-only manual COWs, I=g_true]")
    ...
    if i < 3 and wA0_cow is not None and wApp_cow is not None and wAS_cow is not None:
        outdir_cow = f"plots/{args.polynomial}/{name}/reference_cow_Ig_realistic_weighted_signal_only_manual_1"
    ...
    # Plot the result
    if i < 3:
        outdir_fit = f"plots/{args.polynomial}/{name}/fit_projections_cow_Ig_realistic_weighted_signal_only_manual"
